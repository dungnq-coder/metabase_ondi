...
def print_databases_list(data: dict):
    ...
    for i, db in enumerate(databases, start=1):
        ...
        print(f'    ├── Auto Run Query: {icon_bool(db.get("auto_run_queries"))}')
        ...
        print(f'    ├── Permissions   : {db.get("native_permissions", "N/A").capitalize()}')
        print(f'    ├── Created At    : {format_time(db.get("created_at"))}')
        print(f'    ├── Updated At    : {format_time(db.get("updated_at"))}')
        print(f'    ├── Sync Schedule : {db.get("metadata_sync_schedule", "—")}')
        print(f'    ├── Cache Refresh : {db.get("cache_field_values_schedule", "—")}')
        print(f'    └── Features      : {len(db.get("features", []))} supported\n')
...
def print_database_summary(db):
    ...
    print(f'⚙️  Auto Run Query  : {"✅" if db.get("auto_run_queries") else "❌"}')
