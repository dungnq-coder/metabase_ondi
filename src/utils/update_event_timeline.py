...
def build_bullets(description: str, max_len: int = 255) -> str:
    ...
    return bullets if len(bullets) <= max_len else bullets[: max_len - 3] + '...'
...
def clear_and_add_events(
    events: list[dict], timeline_id: int, patch_note_url: str, manager: MetabaseAPIManager
):
    """Xóa event cũ rồi tạo mới."""
    for ev in events:
        manager.timeline_event.delete_specific_timeline_event(timeline_event_id=ev['id'])
...
def create_new_timeline(
    manager: MetabaseAPIManager, collection_id: int, name: str, icon: str, description: str
):
    payload = manager.timeline.get_update_timeline_payload(
        collection_id=collection_id, description=description, name=name, icon=icon
    )

    res = manager.timeline.post_timeline_action(payload=payload).json()
    print(f"✅ Created new timeline '{name}' (ID={res.get('id')}) in collection {collection_id}")
    return res
...
def add_events(config_pair: tuple, manager: MetabaseAPIManager, timeline_label: str):
...
def add_event_4_FS(config: BaseConfig, manager: MetabaseAPIManager):
    add_events(config.fs_timeline_event, manager, 'FS')
